...
<body>
    ...
    <section id="contact" class="contact mt-3">
      <div class="container" id="firstForm">
        <form action="<%=application.getContextPath()%>/photo/update" method="post" role="form" onsubmit="return writePhoto()" enctype="multipart/form-data">
				...
				<div id = "addDiv">
				<div id = "cloneForm0" class="row align-self-center border pt-3" style = "">
          			<div class="col-lg-6 text-center mb-4 fancybox" style=" position: relative; height: 300px; color: #1bac91;">
            
             		<input type="hidden" id="gameToken" name="list[0].aimage" value="${photo.list[0].aimage}">
				...
				<c:forEach var="i" begin="1" end="9">
					...
					<input type="hidden" id="gameToken" name="list[${i }].aimage" value="${photo.list[i].aimage}">
          			<div class="col-lg-6 text-center mb-4 fancybox" style=" position: relative; height: 300px; color: #1bac91;">
		...
		<div class ="row mb-3 fixed-bottom clearfix">
			<div class = "mx-auto">
				...
				<button type="submit" class="btn ml-2 btn-info">수정하기</button>
			</div>
		</div>
  ...
  $(document).ready(function(){
	  var psize = '${photo.psize}';
	  var ptype = '${photo.ptype}';
	  var pstyle = '${photo.pstyle}';
	  var count = 0;
	  
	  $('#ptype').val(ptype).prop("selected",true);
	  $('#pstyle').val(pstyle).prop("selected",true);
	  $("#psize").val(psize).prop("selected", true);
	  
	  <c:forEach var="photo" items="${photo.list}">
	  $('#cloneForm'+count).attr('style', '');
	  
	  var alocation = '${photo.alocation}';
	  console.log(alocation);
	  console.log('#list['+count+'].alocation');
